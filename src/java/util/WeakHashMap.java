...
public class WeakHashMap<K,V>
     ...
     */
    static Object unmaskNull(Object key) {   //null key的判断
    ...
    private void expungeStaleEntries() {
        for (Object x; (x = queue.poll()) != null; ) {
            synchronized (queue) {  //注意：擦除引用的过程是同步的。
                @SuppressWarnings("unchecked")
                ...
                Entry<K,V> p = prev;
                while (p != null) {
                    Entry<K,V> next = p.next; //p为移动指针。prev为删除e的定位引用
                    if (p == e) {  //question:为什么p不是引用了prev吗？为啥还要去判断prev==e?
                        if (prev == e)
                            table[i] = next;
                        else
                            prev.next = next;
    ...
    public V put(K key, V value) {
        Object k = maskNull(key);
        int h = hash(k);
        Entry<K,V>[] tab = getTable(); //在获取table的时候就进行了无引用的擦除，与ThreadLocalMap不同的是，
                                       // ThreadLocalMap的在擦除前有一些其他操作，擦除也是有条件的。并且ThreadLocalMap是使用的线性探测来做的。
    ...
    void resize(int newCapacity) {
        Entry<K,V>[] oldTable = getTable(); //进行了一次擦除操作
    ...
    private static class Entry<K,V> extends WeakReference<Object> implements Map.Entry<K,V> {
        ...
        public int hashCode() {
            K k = getKey();
            V v = getValue();
            return Objects.hashCode(k) ^ Objects.hashCode(v);//weakHashMap的哈希code算法
        }
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K,V>> {
        ...
        private List<Map.Entry<K,V>> deepCopy() {  //深度拷贝
