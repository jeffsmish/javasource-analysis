 ...
 * 作用:此类的典型用法是拓扑保留对象图形转换，如序列化或深层复制。要执行这样的转换，程序必须维护用于跟踪所有已处理对象引用的“节点表”。节点表一定不等于不同对象，即使它们偶然相等也如此。此类的另一种典型用法是维护代理对象。
 * 注意，此实现不是同步的。可以用synchronizedMap去实现同步。
 * @param <K>
 * @param <V>
 */
    ...
    implements Map<K,V>, java.io.Serializable, Cloneable
{
    ...
    private static int hash(Object x, int length) {
        int h = System.identityHashCode(x); //本地的hash算法。
    ...
    public V put(K key, V value) {
        Object k = maskNull(key);
        Object[] tab = table;  //对象数组 ，一个Entry组成：tab[i]放入key，tab[i+1]放入值
        int len = tab.length;
        int i = hash(k, len); //采用identityHashCode的hash算法，其值相同的对象，引用不同，算出来的值是不同的。
        ...
        while ( (item = tab[i]) != null) {
            ...
            i = nextKeyIndex(i, len);       //采用的是线性探测法
        }
    ...
    private void resize(int newCapacity) {
        ...
        int newLength = newCapacity * 2;  //新数组的长度为原来的两倍

        Object[] oldTable = table;
        int oldLength = oldTable.length;
        if (oldLength == 2*MAXIMUM_CAPACITY) { // can't expand any further 旧数组是否超过最大的长度
            if (threshold == MAXIMUM_CAPACITY-1)   //是否超过最大的负载因子
                throw new IllegalStateException("Capacity exhausted.");
            threshold = MAXIMUM_CAPACITY-1;  // Gigantic map! //是否超过最大的负载因子
            return;
        }
        if (oldLength >= newLength)
            return;

        Object[] newTable = new Object[newLength];//在一切合法验证过后，新数组空间申请
        threshold = newLength / 3;  //新的负载因子计算

        for (int j = 0; j < oldLength; j += 2) {//循环获取旧的数组，注意：步长是2
    ...
    public V remove(Object key) {
        ...
        while (true) {
            Object item = tab[i];
            if (item == k) {//为了防止引用算出来的hashCode一样，但对象不一样的情况。
                modCount++;
                size--;
                @SuppressWarnings("unchecked")
                    V oldValue = (V) tab[i + 1];
     ...
     */
    //删除key和value同时相等的实体，这个方法在转成Set集合后，内部的remove方法调用的。
    ...
    private void closeDeletion(int d) {
        ...
        for (int i = nextKeyIndex(d, len); (item = tab[i]) != null;
            ...
            if ((i < r && (r <= d || d <= i)) || (r <= d && d <= i)) {
                tab[d] = item;
                tab[d + 1] = tab[i + 1];   //将冲突的实体复制迁移到删除实体的空位
                tab[i] = null;            //释放冲突迁移后的空位。
                tab[i + 1] = null;
                d = i;
            }
        }
